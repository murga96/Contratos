...
export const ContractTypes = () => {
  ...
  const [editDialog, dispatch] = useStateValue()
  const filters = {
  ...
  const ElementDialog = (tipoContrato, encabezado, ambasPartes) => {
    return (
      <Dialog
        ...
        modal
        className="p-fluid"
        onHide={() => dispatch({ type: actionTypes.SET_EDIT_DIALOG, editDialog: false })}
      >
        <form onSubmit={handleSubmit(handle)}>
          ...
          <Button
            ...
            type="submit" /*  onClick={saveElement}  */
          />
          <Button
            label="Cancelar"
            icon="pi pi-check"
    ...
    return (
      <div>
        <Table
          ...
          exportData={
            true
          } /* ElementDialog={ElementDialog(null, null, null)} h1={elementDialog} */
        />
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      {error && <h5>{error}</h5>}
      {!(loading || error) ? renderData() : console.log("object")}
    </div>
  );
};
