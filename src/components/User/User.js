...
export const Users = () => {
  ...
  const schema = yup.object().shape({
    "nombreUsuario": yup.string().strict(true).required("Nombre de usuario es requerido").min(5, "Nombre de usuario debe tener al menos 5 carácteres")
    .lowercase("Nombre de usuario debe ser en minúsculas").trim("Nombre de usuario no puede contener espacios"),
  ...
  let dataStruct = [
    ...
    },
    {
      ...
      name: "idEjecutivo",
      defaultValue: 1,
      props: {options: ejecutivos.data?.findAllEjecutivos, filter: true,  optionLabel: "nombre", optionValue: "idEjecutivo", placeholder: "Selecciona un ejecutivo"}
    },
    {
      id: 5,
      component: "label",
    ...
    },
    {
      ...
      name: "roles",
      defaultValue: [],
      props: {options: roles.data?.findAllRoles,  optionLabel: "rol", optionValue: "idRol", maxSelectedLabels:3 , placeholder: "Selecciona los roles"}
    },
  ];
  const formProps = {"data": dataStruct, "schema": schema, "handle": updateTC, "variables": { usuario: {} }, "buttonsNames": ["Guardar", "Cancelar"]}
  return (
    <div>
      <Navbar />
      ...
      {!(loading || error || loadingU || errorU) ? (
        <div>
          <Table
            ...
            additionalButton={[<Button icon="pi pi-unlock" className="p-button-rounded mr-2" data-pr-tooltip="Forzar contraseña"/>, forzarPassword]}
          />
        </div>
      ) : (
        //poner cargar
